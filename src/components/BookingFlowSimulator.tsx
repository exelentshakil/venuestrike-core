...
export function BookingFlowSimulator() {
  ...
  return (
    <div className="w-full rounded-2xl border border-slate-300 dark:border-slate-800 bg-[var(--color-surface)] p-5 sm:p-7 shadow-xs">
      {/* Top Banner Header with Problem-Solution Context */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-rose-50 dark:bg-rose-950/50 px-3 py-1 text-xs font-extrabold text-rose-950 dark:text-rose-200 border border-rose-300 dark:border-rose-800 whitespace-nowrap shrink-0 shadow-xs font-mono">
              <Flame className="h-3.5 w-3.5 text-rose-600 dark:text-rose-400" />
              Peak Saturday Concurrency Crisis
            </span>
            <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 dark:bg-emerald-950/50 px-3 py-1 text-xs font-extrabold text-emerald-950 dark:text-emerald-200 border border-emerald-300 dark:border-emerald-800 whitespace-nowrap shrink-0 shadow-xs font-mono">
              <ShieldCheck className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-400" />
              Zero Peak Double-Bookings
            </span>
            <span className="text-xs font-mono font-bold text-slate-700 dark:text-slate-300 hidden sm:inline">
              Austin Downtown &bull; 16 Bowling Lanes &bull; 8 Darts &bull; 2 VIP Lounges
            </span>
          </div>
          <h2 className="text-xl sm:text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">
            Saturday Night Concurrency Simulator &amp; Motion Visual Guide
          </h2>
          <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 mt-1 max-w-4xl leading-relaxed font-normal">
            A real-world simulation of peak Saturday night rush: <strong>12 walk-in guests at the front-desk iPad</strong> competing with <strong>49 simultaneous online mobile checkouts</strong> for the exact same lane. See how PostgreSQL transaction advisory locks (<code className="font-bold text-indigo-700 dark:text-indigo-300">pg_try_advisory_xact_lock</code>) and 120s Redis holds guarantee exactly 1 winner and 0 double bookings.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5 shrink-0 flex-wrap">
          <Button
            ...
            className="h-10 px-4 bg-gradient-to-r from-rose-600 via-indigo-600 to-violet-700 text-white font-bold text-xs rounded-xl shadow-md hover:opacity-95 transition-all whitespace-nowrap shrink-0 border border-rose-400"
          >
            {isSimulatingRace ? (
              <>
      ...
      <div className="mt-6 rounded-2xl border-2 border-indigo-200 dark:border-indigo-900 bg-gradient-to-b from-indigo-50/70 via-white to-slate-50/50 dark:from-indigo-950/30 dark:via-slate-900 dark:to-slate-950 p-5 sm:p-6 shadow-sm">
            ...
            return (
              <button
                ...
                className={`p-3.5 rounded-xl border text-left transition-all relative overflow-hidden flex flex-col justify-between ${
                  isActive
                    ? 'border-indigo-600 bg-white dark:bg-slate-900 shadow-md ring-2 ring-indigo-500/30'
                    : 'border-slate-300 dark:border-slate-800 bg-slate-100/80 dark:bg-slate-900/40 hover:border-slate-400 hover:bg-white dark:hover:bg-slate-900 cursor-pointer'
                }`}
              >
        ...
        <div className="rounded-xl border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-xs space-y-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-3">
            <div>
              <div className="text-[11px] font-mono font-bold uppercase tracking-wider text-indigo-700 dark:text-indigo-400 mb-1">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className="text-xs font-mono font-bold px-2.5 py-1 rounded-md bg-emerald-50 dark:bg-emerald-950/50 text-emerald-900 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800">
                P99 Latency: 3.4ms
              </span>
            </div>
          </div>
          ...
          {/* Animated SVG Visual Architecture Pipeline */}
          <div className="relative rounded-xl border border-slate-300 dark:border-slate-800 bg-slate-950 text-white p-4 overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center py-2">
              ...
              <div className={`p-3 rounded-lg border text-center transition-all ${
                ...
                <div className="text-xs font-bold text-indigo-300 mb-1 flex items-center justify-center gap-1.5">
              ...
              <div className={`p-3 rounded-lg border text-center transition-all ${
                ...
                <div className="text-xs font-bold text-emerald-300 mb-1 flex items-center justify-center gap-1.5">
            ...
            <div className="mt-3 pt-2.5 border-t border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-[11px] font-mono">
              <div className="text-slate-300">
                <span className="text-amber-400 font-bold">Mechanism: </span>
                {currentPhase.technicalMechanism}
              </div>
              <div className="text-rose-300 shrink-0 font-bold">
      ...
      {raceResults?.aiDiagnosis && (
        <div className="mt-6 rounded-2xl border-2 border-emerald-300 dark:border-emerald-800 bg-gradient-to-r from-emerald-50/90 via-white to-indigo-50/70 dark:from-emerald-950/40 dark:via-slate-900 dark:to-indigo-950/30 p-5 shadow-sm space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-emerald-200 dark:border-emerald-900/60 pb-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-extrabold uppercase tracking-wider text-emerald-950 dark:text-emerald-200 font-mono">
                    AI Concurrency Post-Mortem &amp; Operator Diagnosis
                  </span>
                  <Badge className="bg-emerald-700 text-white font-mono text-[10px] px-2 py-0 font-bold">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className="text-xs font-mono font-bold text-emerald-950 dark:text-emerald-200 bg-emerald-100 dark:bg-emerald-950 px-2.5 py-1 rounded-md border border-emerald-300 dark:border-emerald-800">
                Prevented Loss: $1,200 / Incident
              </span>
            </div>
          </div>

          {/* 4 High-Signal Callout Blocks */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 text-xs font-mono">
            <div className="p-3.5 rounded-xl border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs space-y-1">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">
                01 &bull; Saturday Crisis Context
              </span>
              <p className="text-slate-800 dark:text-slate-200 font-sans font-medium text-xs leading-relaxed">
          ...
          <div className="p-3 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-300 dark:border-indigo-800 text-xs font-mono text-indigo-950 dark:text-indigo-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <strong className="font-extrabold text-indigo-900 dark:text-indigo-300">Operator Strategic Recommendation: </strong>
              <span>{raceResults.aiDiagnosis.operatorRecommendation}</span>
            </div>
            <span className="text-[11px] font-bold text-indigo-700 dark:text-indigo-400 shrink-0">
      ...
      {/* ========================================================= */}
      {/* MAIN TWO-COLUMN WORKBENCH: FLOOR MAP + RESERVATION ENGINE */}
      {/* ========================================================= */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pt-6">
        ...
        <div className="lg:col-span-7 space-y-5">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-mono text-slate-500 font-semibold">
                16 Lanes &bull; 4 Darts &bull; 2 Lounges
              </span>
            </div>
          ...
          {/* Floor Map Cards - Sleek, Uncluttered, Guaranteed Non-Colliding Layout */}
              ...
              return (
                <button
                  ...
                  className={`p-3 rounded-xl border text-left transition-all relative overflow-hidden flex flex-col justify-between min-h-[110px] ${
                    isTargetLane
                      ? 'border-2 border-indigo-600 bg-indigo-50 dark:bg-indigo-950/50 ring-2 ring-indigo-500/30 shadow-md'
                      : isSelected
                      ? 'border-indigo-600 bg-indigo-50/70 dark:bg-indigo-950/40 ring-2 ring-indigo-500/20 shadow-xs'
                      : isAvailable
                      ? 'border-slate-300 dark:border-slate-800 bg-[var(--color-panel-subtle)] hover:border-slate-400 hover:bg-white dark:hover:bg-slate-900 cursor-pointer'
                      : isHeld
                      ? 'border-amber-300 dark:border-amber-800/80 bg-amber-50/50 dark:bg-amber-950/20 opacity-90 cursor-not-allowed'
                      : 'border-slate-200 dark:border-slate-850 bg-slate-100 dark:bg-slate-900/50 opacity-60 cursor-not-allowed'
                  }`}
                >
                  <div className="flex items-center justify-between w-full">
                    ...
                    <Badge
                      variant="outline"
                      className={`text-[9px] px-1.5 py-0 font-mono uppercase font-bold ${
                        isTargetLane
                  ...
                  {/* Single-row metadata to completely eliminate vertical text collision */}
                  <div className="flex items-center justify-between gap-1 mt-1 text-[11px] font-mono">
                    ...
                    {item.divertedCount && item.divertedCount > 0 && (
                      <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-950/60 px-1.5 py-0.2 rounded border border-emerald-300 dark:border-emerald-800 shrink-0">
                        +{item.divertedCount}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between text-[10px] text-slate-500 font-mono pt-1 border-t border-slate-200 dark:border-slate-800">
                    ...
                    {isTargetLane ? (
                      <span className="text-indigo-700 dark:text-indigo-400 font-bold flex items-center gap-0.5">
          ...
          {/* Real 50-Thread Audit Trace Terminal */}
          <div className="rounded-xl border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 space-y-3 shadow-xs">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 dark:border-slate-800 pb-3">
              ...
              <div className="flex items-center gap-2">
                <div className="flex rounded-lg border border-slate-300 dark:border-slate-800 p-0.5 text-[11px] font-mono">
                  <button
                    onClick={() => setTraceFilter('all')}
                    className={`px-2 py-0.5 rounded ${traceFilter === 'all' ? 'bg-indigo-600 text-white font-bold' : 'text-slate-600 dark:text-slate-400'}`}
                  >
                    All (50)
                  </button>
                  <button
                    onClick={() => setTraceFilter('winner')}
                    className={`px-2 py-0.5 rounded ${traceFilter === 'winner' ? 'bg-emerald-600 text-white font-bold' : 'text-slate-600 dark:text-slate-400'}`}
                  >
                    Winner (1)
                  </button>
                  <button
                    onClick={() => setTraceFilter('rerouted')}
                    className={`px-2 py-0.5 rounded ${traceFilter === 'rerouted' ? 'bg-indigo-600 text-white font-bold' : 'text-slate-600 dark:text-slate-400'}`}
                  >
                    Rerouted (49)
                  </button>
                </div>
                ...
                return (
                  <div
                    ...
                    className={`p-2 rounded-lg border flex items-center justify-between gap-2 ${
                      isAcquired
                        ? 'border-emerald-400 bg-emerald-50/80 dark:bg-emerald-950/40 text-emerald-950 dark:text-emerald-200'
            ...
            <div className="pt-2 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-[11px] font-mono text-slate-500">
              <span>Showing {Math.min(15, filteredThreads.length)} of {filteredThreads.length} audit records</span>
              <span className="text-indigo-700 dark:text-indigo-400 font-bold">PostgreSQL Advisory Lock Hash: 8492049182390</span>
            </div>
          </div>
        </div>

        {/* Right Column: Reservation Config & Package Bundler (5 cols) */}
        <div className="lg:col-span-5 rounded-xl border border-slate-300 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900/60 p-4 sm:p-5 space-y-4 flex flex-col justify-between shadow-xs">
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              ...
              <Badge variant="outline" className="text-xs font-mono font-bold bg-white dark:bg-slate-900 border-indigo-400 text-indigo-700 dark:text-indigo-300">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <div className="px-2.5 py-1.5 rounded-lg border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 text-xs font-mono font-bold flex items-center gap-1.5 text-slate-900 dark:text-white">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="h-8 w-8 rounded-lg border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 text-xs font-bold hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-900 dark:text-white"
                  >
                    -
                  </button>
                  <span className="text-xs font-bold font-mono text-center flex-1 text-slate-900 dark:text-white">
                    {partySize} Guests
                  </span>
                  <button
                    ...
                    <div
                      ...
                      className={`p-3 rounded-xl border cursor-pointer transition-all ${
                        isSelected
                          ...
                          : 'border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 hover:border-slate-400'
                      }`}
                    >
                      ...
                      <div className="mt-1 text-[11px] text-slate-600 dark:text-slate-400 leading-tight">
            ...
            <div className="rounded-xl border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 p-3.5 space-y-2 text-xs font-mono">
              <div className="flex items-center justify-between text-slate-600 dark:text-slate-400">
                <span>Play &amp; F&amp;B Subtotal ({partySize} x ${activePkg.pricePerPerson})</span>
                <span className="font-bold text-slate-900 dark:text-white">${totalCost}.00</span>
              </div>
              <div className="flex items-center justify-between text-indigo-700 dark:text-indigo-400 font-extrabold pt-1 border-t border-slate-200 dark:border-slate-800">
              ...
              <div className="mt-2 pt-2 border-t border-dashed border-slate-300 dark:border-slate-800 text-[11px] text-emerald-700 dark:text-emerald-400 flex items-center justify-between font-bold">
          ...
          <div className="space-y-2 pt-2">
            {isHolding ? (
              <div className="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-800 text-xs font-mono space-y-1.5 shadow-xs">
                <div className="flex items-center justify-between text-amber-900 dark:text-amber-300 font-extrabold">
                  <span className="flex items-center gap-1.5">
                    <Lock className="h-4 w-4 text-amber-600" />
                    Slot Exclusively Held (Redis TTL)
                  </span>
                  <span className="text-sm font-extrabold bg-amber-200 dark:bg-amber-900 px-2 py-0.5 rounded">
                    {holdTimer}s
                  </span>
                </div>
                <div className="text-[11px] text-amber-800 dark:text-amber-400 truncate">
                  Lock Token: <span className="font-bold">{activeLockToken || 'VS_LOCK_SAT8PM_01E110'}</span>
                </div>
              ...
              <Button
                onClick={handleSimulatePeakRush}
                className="w-full h-10 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-xs transition-colors whitespace-nowrap shrink-0"
              >
