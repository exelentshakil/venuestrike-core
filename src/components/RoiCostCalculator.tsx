...
export function RoiCostCalculator() {
  ...
  const [infraTier, setInfraTier] = useState<'standard' | 'enterprise'>('enterprise');
  ...
  const awsRdsPostgres = infraTier === 'enterprise' ? 75 : 45; // db.t4g.medium Multi-AZ vs single AZ
    ...
    <div className="w-full rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 sm:p-7 shadow-xs">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[var(--color-border)] pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-[var(--color-text-secondary)] mt-1 max-w-3xl leading-relaxed font-normal">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="px-3 py-1.5 rounded-lg border border-[var(--color-border)] bg-[var(--color-panel-subtle)] text-xs font-mono">
            ...
            <span className="text-emerald-600 dark:text-emerald-400 font-semibold">${totalMonthlyInfra} / mo</span>
          </div>
        </div>
      </div>

      {/* Grid: Inputs (Left) + Economics & Cloud Itemization (Right) - items-stretch for vertical parity */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 pt-5 items-stretch">
        {/* Left Inputs (5 cols) - flex flex-col justify-between to eliminate bottom void */}
        <div className="lg:col-span-5 rounded-xl border border-[var(--color-border)] bg-[var(--color-panel-subtle)] p-4 sm:p-5 flex flex-col justify-between shadow-xs">
          <div className="space-y-4">
            ...
            <div className="space-y-3.5 text-xs">
              <div>
                <div className="flex justify-between text-xs font-medium mb-1">
                  ...
                  <span className="font-mono text-indigo-600 dark:text-indigo-400 font-semibold">{monthlyBookings.toLocaleString()}</span>
                </div>
                <input
                  type="range"
                  min="300"
              ...
              <div>
                <div className="flex justify-between text-xs font-medium mb-1">
                  ...
                  <span className="font-mono text-emerald-600 dark:text-emerald-400 font-semibold">${avgBookingValue}</span>
                </div>
                <input
                  type="range"
                  min="80"
              ...
              <div>
                <div className="flex justify-between text-xs font-medium mb-1">
                  ...
                  <span className="font-mono text-purple-600 dark:text-purple-400 font-semibold">{fnbAttachRate}%</span>
                </div>
                <input
                  type="range"
                  min="10"
              ...
              <div>
                <div className="flex justify-between text-xs font-medium mb-1">
                  <span className="text-[var(--color-text-secondary)]">Peak Collisions Prevented</span>
                  <span className="font-mono text-rose-600 dark:text-rose-400 font-semibold">{preventedCollisions} incidents / mo</span>
                </div>
                <input
                  type="range"
                  min="2"
            ...
            <div className="pt-3 border-t border-[var(--color-border)] text-xs font-mono text-[var(--color-text-secondary)] space-y-1.5">
              <div className="flex justify-between">
                <span>Gross Monthly Volume:</span>
                <span className="font-semibold text-[var(--color-text-primary)]">${grossMonthlyVolume.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span>Attributed F&amp;B Catering:</span>
                <span className="font-semibold text-emerald-600 dark:text-emerald-400">${Math.round(fnbRevenue).toLocaleString()}</span>
              </div>
            </div>
          </div>

          {/* Institutional Mutex SLA Guarantee - anchors bottom of left column */}
          <div className="mt-4 pt-3.5 border-t border-[var(--color-border)] flex items-center justify-between text-xs font-mono text-[var(--color-text-secondary)]">
            <span className="flex items-center gap-1.5 text-emerald-700 dark:text-emerald-400 font-medium">
              ...
              SLA: 0.00% Double-Bookings
            </span>
            <span className="text-xs text-[var(--color-text-muted)] font-medium">PostgreSQL Lock Mutex</span>
          </div>
        </div>

        {/* Right Outputs & Itemized Cloud Bill (7 cols) - flex flex-col justify-between */}
        <div className="lg:col-span-7 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 sm:p-5 flex flex-col justify-between shadow-xs">
          <div>
            <span className="text-xs font-semibold uppercase tracking-wider text-[var(--color-text-primary)] font-mono block border-b border-[var(--color-border)] pb-2.5 mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
              <div className="p-3.5 rounded-xl bg-emerald-50/70 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800">
                <span className="text-xs font-mono font-semibold uppercase tracking-wider text-emerald-800 dark:text-emerald-300 block mb-1">
                  Walkout Losses Prevented
                </span>
                <div className="text-2xl font-semibold font-mono text-emerald-700 dark:text-emerald-300">
                ...
                <span className="text-xs text-[var(--color-text-secondary)] block mt-1 leading-relaxed">
                  Saved refunds, comped drink tabs, and customer churn
                </span>
              </div>

              <div className="p-3.5 rounded-xl bg-indigo-50/70 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800">
                <span className="text-xs font-mono font-semibold uppercase tracking-wider text-indigo-800 dark:text-indigo-300 block mb-1">
                  Net Incremental Value
                </span>
                <div className="text-2xl font-semibold font-mono text-indigo-700 dark:text-indigo-300">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between p-2.5 rounded-lg bg-[var(--color-panel-subtle)] border border-[var(--color-border)]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-[var(--color-text-primary)]">AWS RDS PostgreSQL ({infraTier === 'enterprise' ? 'db.t4g.medium Multi-AZ' : 'db.t4g.small Single-AZ'})</span>
                  </div>
                  <span className="font-semibold text-[var(--color-text-primary)]">${awsRdsPostgres} / mo</span>
                </div>
                <div className="flex items-center justify-between p-2.5 rounded-lg bg-[var(--color-panel-subtle)] border border-[var(--color-border)]">
                  <div className="flex items-center gap-2">
                    <Zap className="h-3.5 w-3.5 text-purple-600 shrink-0" />
                    <span className="font-medium text-[var(--color-text-primary)]">AWS ElastiCache Redis (120s TTL Mutex Holds)</span>
                  </div>
                  <span className="font-semibold text-[var(--color-text-primary)]">${awsElastiCacheRedis} / mo</span>
                </div>
                <div className="flex items-center justify-between p-2.5 rounded-lg bg-[var(--color-panel-subtle)] border border-[var(--color-border)]">
                  <div className="flex items-center gap-2">
                    <Cpu className="h-3.5 w-3.5 text-emerald-600 shrink-0" />
                    <span className="font-medium text-[var(--color-text-primary)]">Vercel Fluid Compute &amp; Edge Network</span>
                  </div>
                  <span className="font-semibold text-[var(--color-text-primary)]">${vercelFluidCompute} / mo</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 pt-3.5 border-t border-[var(--color-border)] flex items-center justify-between text-xs font-mono text-[var(--color-text-secondary)]">
            <span className="text-emerald-700 dark:text-emerald-400 font-semibold">ROI Ratio: {Math.round(netValueGenerated / totalMonthlyInfra)}x Return</span>
